...
class AntRobot:
    ...
    def define_ant(self):
        ...
        for ind, limb in enumerate(self.limbs):
            ...
            parent_xml = ant_xml
            xml.SubElement(parent_xml, "geom",
                           attrib={"name": f"limb{ind}",
                                   "type": "capsule",
    ...
    def define_actuators(self):
        ...
        for motor_name in self.motor_refs:
            if self.verbose:
                print(motor_name)
            xml.SubElement(actuators_xml, "motor", attrib={"gear": self.properties["joint"]["gear"],
...
def default_setting(props=properties):
    ...
    xml.SubElement(default, "geom",
                   attrib={"condim": props["geom"]["condim"],
