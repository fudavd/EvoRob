...
from src.utils.geometry import quat_rel_vecs
...
class PassiveWalkerRobot:
    def __init__(self, points=NDArray, connectivity_mat=NDArray, joint_limits=None, name: str = "PassiveWalkerRobot", props=None,
                 fixed_base=False, verbose=True, z_offset=0.0,):
        if props is None:
            props = properties
        self.properties = props
        self.options = {"timestep": "1e-3",
                        "integrator": "RK4"}
        ...
        self.offset = np.array([0, 0, float(props["geom"]["margin"])*1.5 +
                                  radius - np.min(points[:, 2]) + z_offset])
        self.points = points
        ...
        self.connect = [np.argwhere(self.connectivity_mat[ind] == np.inf).tolist() for ind in range(self.n_points)]
    ...
    def define_walker(self):
        ...
        walker_xml = xml.SubElement(worldbody_xml, "body",
                                    attrib={"name": f"Base",
        ...
        for ind, limb in enumerate(self.limbs):
            ...
            for segment in segments:
                ...
                segment_cog += rel_xyz
                segment_xml = xml.SubElement(parent_xml, "body",
                                             attrib={"name": rod_name,
                                                     "pos": f"{segment_cog[0]} {segment_cog[1]} {segment_cog[2]}"})

                xml.SubElement(segment_xml, "geom",
                               attrib={"name": rod_name.replace("rod", "geom"),
                                       "type": "capsule",
                ...
                xml.SubElement(segment_xml, "site",
                               attrib={"name": rod_name.replace("rod", "site"),
                                       "pos": f"{0} {0} {0}",
                ...
                if segment[0] in self.joints:
                    ...
                    joint_pos = -rel_xyz
                    xml.SubElement(segment_xml, "joint",
                    ...
                    sphere_xml = xml.SubElement(segment_xml, "body",
                                                attrib={"name": point_name,
                                                        "pos": f"{joint_pos[0]} {joint_pos[1]} {joint_pos[2]}"})
                    xml.SubElement(sphere_xml, "geom", attrib={"type": self.properties["connect"]["type"],
                ...
                segment_cog = rel_xyz
                if self.verbose:
                    print(f"{rod_name}:\t{np.linalg.norm(xyz_to - xyz_from)} " + joint_name)
    ...
    def define_actuators(self):
        ...
        for motor in self.motors:
            gear = self.connectivity_mat[motor, motor]
                ...
                for base_segment in limb[0]:
                    if base_segment[1] == motor:
                        ...
                        for segment in limb[0]:
                            ...
                            xml.SubElement(actuators_xml, "joint", attrib={"type": self.properties["actuator"]["type"],
...
def default_setting(props=properties):
    ...
    xml.SubElement(default, "joint",
                   attrib={"armature": props["joint"]["armature"],
                           "damping": props["joint"]["damping"],
                           "type": "hinge",
                           "range": props["joint"]["range"],
                           "axis": props["joint"]["axis"],})
    xml.SubElement(default, "geom",
                   attrib={"condim": "3",
...
if __name__ == "__main__":
    ...
    points = np.array([ 0., -0.05, 0.0,   0, -0.05, -0.35,   0, -0.05, -0.65,   0.1, -0.05, -0.65,  .1,  0.02, -0.65,
                        0.,  0.05, 0.0,   0,  0.05, -0.35,   0,  0.05, -0.65,   0.1,  0.05, -0.65,  .1, -0.02, -0.65,])
    ...
    joint_limits = [[-45, 45], [-150, 0], [-45, 45], [-150, 0],]
    test = PassiveWalkerRobot(points.reshape(len(points) // 3, 3), connectivity_mat, joint_limits)
